  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        console.log(
          `[adjustImage] Cropping to ${size}x${size} from (${startX}, ${startY})`,
        );
        ...
        canvas.toBlob(
          (blob) => {
            ...
            console.log("[adjustImage] New file created:", {
              name: newFile.name,
              size: newFile.size,
              type: newFile.type,
            });
          ...
          file.type || "image/png",
          0.9,
        ); // added quality parameter for potential size reduction
      };
      img.onerror = (err) => {
...
const PostForm: React.FC<PostFormProps> = ({
  ...
  // プラットフォーム別の文字数監視と検証
  const validatePlatformContents = (checkEmpty = false) => {
    ...
    selectedPlatforms.forEach((platform) => {
      ...
      // 0文字（空）の場合のバリデーション（checkEmptyがtrueの時のみ）
      if (checkEmpty && contentLength === 0) {
        if (!errors[platform]) errors[platform] = [];
        errors[platform].push(
          `${validation.name}の投稿内容を入力してください。`,
        );
      } else if (contentLength > validation.maxLength) {
        if (!errors[platform]) errors[platform] = [];
        errors[platform].push(
  ...
  React.useEffect(() => {
    if (selectedPlatforms.length > 0) {
      validatePlatformContents(false);
    }
  ...
  const handleFormSubmit = async (isDraft = false) => {
    ...
    try {
      // 下書き保存時も投稿確定時も0文字チェックと文字数超過チェックを実行
      if (!validatePlatformContents(true)) {
        // 0文字チェックも実行
        toast({
          title: "入力エラー",
          description:
            "投稿内容が入力されていないプラットフォームがあるか、文字数制限を超えています。",
          variant: "destructive",
        });
        return;
      }
        ...
        for (const file of allNewFilesToUpload) {
          ...
          if (uploadResult.success) {
            ...
            if (selectedPlatforms.includes("instagram")) {
              try {
                addLogEntry("INFO", "Adjusting image for Instagram", {
                  originalFile: {
                    name: file.name,
                    size: file.size,
                    type: file.type,
                ...
                addLogEntry("INFO", "Image adjusted for Instagram", {
                  adjustedFile: {
                    name: instagramImage.name,
                    size: instagramImage.size,
                    type: instagramImage.type,
                  },
                });

                const dropboxResult = await uploadImageToDropbox(
                  instagramImage,
                  uploadResult.imageId,
                );
                if (dropboxResult.success) {
                  // Google SheetsのDropbox URLカラムを更新
                  await updateImageDropboxUrl(
                    uploadResult.imageId,
                    dropboxResult.directUrl,
                  );
                  addLogEntry(
                    "INFO",
                    "Image also uploaded to Dropbox for Instagram",
                    {
                      imageId: uploadResult.imageId,
                ...
                addLogEntry(
                  "ERROR",
                  "Dropbox upload error including image processing",
                  {
                    imageId: uploadResult.imageId,
                    error:
    ...
    selectedPlatforms.forEach((platform) => {
      const platformSelectedImages = platformImages[platform] || [];
      const platformImageIdsList: string[] = [];
      
      platformSelectedImages.forEach((previewData) => {
        if (previewData.type === "existing") {
          // 既存画像の場合、IDをそのまま使用
          platformImageIdsList.push(previewData.id);
        } else if (previewData.type === "new" && previewData.originalFile) {
          // 新規画像の場合、アップロード済みのIDを探す
          const fileKey = `${previewData.originalFile.name}-${previewData.originalFile.size}`;
          const uploadedIndex = allNewFilesToUpload.findIndex(
            (f) => `${f.name}-${f.size}` === fileKey
          );
          if (uploadedIndex !== -1 && uploadedIndex < newImageIds.length) {
            platformImageIdsList.push(newImageIds[uploadedIndex]);
          }
        }
      });
      
      platformImageIdsMap[platform] = platformImageIdsList;
    });

    // 編集時の画像ID管理を改善
    addLogEntry("INFO", "Image ID management for submission", {
      existingImageIdsFromState: imageIds,
      newUploadedImageIds: newImageIds,
      platformImageIdsMap: platformImageIdsMap,
      isEditing: isEditing,
      selectedPlatforms: selectedPlatforms,
    });
  ...
  const getCurrentStep = (): number => {
    ...
    // Check if BOTH content AND platform content have text (both are required)
    const hasContent = content && content.trim().length > 0;
    const hasPlatformContent = Object.values(platformContent).some(val => val && val.trim().length > 0);
    if (!hasContent || !hasPlatformContent) return 2;
  ...
  const formSteps = [
    ...
    { id: 4, label: '個別スケジュール設定(任意)', description: '日時を設定（任意）' },
          ...
          <Card className="p-4 ylpm-glass-card ylpm-slide-in-up">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-normal text-green-800">AI設定済み</span>
              </div>
              <Button
                variant="outline"
                size="sm"
            ...
            <div className="mt-2 text-sm text-green-700">
              <p>〇サービス: {aiSettings?.ai_service}〇モデル: {aiSettings?.ai_model},
              〇接続状態:{" "}
            ...
            {aiConfigured ? (
              <div className="space-y-3">
                <div className="space-y-2">
                  ...
                  <Textarea
                    id="ai-base-content"
                    placeholder="例：新商品の紹介、イベント告知、キーワードなど...AIが生成する際の基となる内容やキーワードを入力してください。"
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-sm text-muted-foreground mb-1 space-y-1">
                    <p>各プラットフォームに合わせてどのように最適化するか指示してください</p>
                    <div className="p-2 bg-blue-50 border border-blue-200 rounded text-xs">
                      <p className="font-medium mb-1">プラットフォーム別に指示する場合の記入例：</p>
                      <p className="font-mono">x=カジュアルな口調で, instagram=絵文字多めで, facebook=丁寧な説明で, line=親しみやすく, discord=フレンドリーに, wordpress=詳しく説明して</p>
                    </div>
                  </div>
                  <Textarea
                    id="ai-instruction"
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-xs text-orange-600 bg-orange-50 p-2 rounded border border-orange-200">
                    ※生成ボタンを押下すると、下のプラットフォーム別設定の投稿内容に上書します。
                  </div>
                </div>
              </div>
            ) : (
